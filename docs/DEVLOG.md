...
### 숏폼 품질 v3 — 훅/리듬/자막/사진 모션

UI 고도화 이후 병목을 실제 생성 영상 품질로 전환. TTS provider는 당분간 Typecast를 유지하고, 공급자 교체 대신 쇼츠 편집 리듬을 먼저 개선.

변경:
- [x] 첫 장면을 12~24자 훅으로 제한
- [x] '오늘은/소개할게요/안녕하세요' 같은 느린 도입을 프롬프트에서 명시적으로 배제
- [x] 총 대본 권장 길이를 210~280자로 줄여 밀도 향상
- [x] 장면 수를 7~10개 중심으로 조정해 컷 전환 빈도 상승
- [x] 동일 장면 문장 중복 검증 추가
- [x] 장면마다 새 정보 하나를 추가하도록 프롬프트 강화
- [x] 제목 오버레이 노출 시간을 3.4초 → 2.0초로 단축
- [x] 사진 Ken Burns 효과를 줌+미세 팬 조합으로 강화
- [x] 사진 전환 crossfade를 0.18초 → 0.12초로 짧게 조정
- [x] 자막 폰트/패딩을 약간 축소하고 하단 UI와 더 멀리 떨어지도록 위치 상향
- [x] 자막 배경을 덜 무겁게 조정해 화면을 덜 가림

검증 기준:
- 첫 장면이 24자를 넘으면 생성 결과 검증 단계에서 실패
- 완전히 같은 장면 문장은 허용하지 않음
- 자막은 Shorts 하단 인터페이스 안전 영역 위에 위치
- 기존 실제 H.264/AAC 렌더 검증 테스트 유지
