...
사용자 피드백:
- 오른쪽 휴대폰 목업에 실제 생성된 쇼츠가 재생되면 좋겠음
- 히어로/스타일/보이스/생성 섹션의 좌우 정렬을 더 맞출 필요가 있음
- 로컬 실행을 위해 매번 Git Bash에서 Streamlit을 켜는 흐름은 배포 단계에서 제거 예정

변경:
- [x] 세션에 생성 결과가 있으면 해당 MP4를 히어로 우측에 실제 영상으로 재생
- [x] 세션 결과가 없으면 output 폴더에서 가장 최근 MP4를 자동 탐색
- [x] 실제 영상이 하나도 없을 때만 기존 휴대폰 목업 표시
- [x] 실제 영상 프리뷰를 9:16 휴대폰 프레임 형태로 표시
- [x] 히어로 좌우 비율/수직 정렬 보정
- [x] 섹션 헤더 설명 정렬 기준 통일
- [x] VOICE 영역을 하나의 패널로 묶어 좌우 기준선 정리
- [x] 모바일에서는 설명을 좌측 정렬로 전환

배포 방향:
- 로컬 개발 중에는 Streamlit 프로세스를 직접 실행
- 실제 서비스 배포 후에는 서버가 상시 실행되므로 사용자는 브라우저 URL만 사용
- GitHub push 기반 자동 배포 구조로 전환 예정


### UI UX 폴리싱 — 네비바/스타일 카드/생성 패널

사용자 화면 검토에서 발견된 문제:
- Streamlit 상단 chrome 때문에 커스텀 네비바가 상단에서 잘려 보임
- 스타일 카드가 시각 요소일 뿐 실제 클릭 대상이 아니고, 별도 라디오 버튼을 눌러야 선택되는 UX
- CREATE 영역이 패딩/여백이 부족해 다른 섹션 대비 완성도가 떨어짐

수정:
- [x] Streamlit 기본 헤더 숨김 및 상단 safe spacing 확보
- [x] AutoShorts 커스텀 네비바가 잘리지 않도록 topbar 높이/여백 보정
- [x] 스타일 라디오 버튼 제거
- [x] 맛집·체험 / 제품 리뷰 / 여행·일상 / 정보형 카드 자체를 실제 버튼으로 변경
- [x] 선택된 스타일 카드는 primary 상태로 즉시 강조
- [x] 스타일 변경 시 내부 음성 캐시/미리듣기도 새 preset 기준으로 초기화
- [x] CREATE 영역을 독립된 패널로 묶고 내부 패딩/CTA 간격/버튼 높이 보정
- [x] URL이 없을 때 버튼 비활성 이유를 안내
- [x] 생성 완료 후 rerun하여 상단 실제 영상 프리뷰도 즉시 갱신

참고한 UX 패턴:
- 영상 생성 서비스의 '입력 → 옵션 카드 → 즉시 CTA → 결과 프리뷰' 흐름
- 카드 전체를 클릭 타깃으로 사용하는 SaaS 온보딩/선택 UI
- 기술 옵션보다 결과 미리보기와 선택 상태를 우선 노출
