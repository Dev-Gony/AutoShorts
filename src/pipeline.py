...
class Pipeline:
    ...
    def run(self, url: str) -> PipelineResult:
        ...
        print("[3/6] TTS 음성 생성")
        audio_path = self.audio_service.synthesize(short_script.script, work_dir / "voice.mp3")

        with AudioFileClip(str(audio_path)) as audio:
            audio_duration = float(audio.duration)
        if audio_duration > settings.max_video_seconds:
            raise RuntimeError(
                f"생성 음성이 {audio_duration:.1f}초로 최대 {settings.max_video_seconds:.0f}초를 초과했습니다."
            )
        ...
        if not backgrounds:
            raise FileNotFoundError(
                "assets/backgrounds 폴더에 MP4 배경 영상을 하나 이상 넣어주세요."
            )
